...
class Token
{
     ...
     * Lists accessors.
     *
     * @see    https://www.vaultproject.io/api/auth/token/index.html#list-accessors
     * @return mixed
     */
    public function listAccessors()
    {
        return $this->client->get('/v1/auth/token/accessors?list=true');
    }
    
    public function lookupAccessor(array $body = [])
    {
        $body = OptionsResolver::resolve($body, ['accessor']);
        $params = ['body' => json_encode($body)];
        return $this->client->post('/v1/auth/token/lookup-accessor', $params);
    }

    public function revokeAccessor(array $body = [])
    {
        $body = OptionsResolver::resolve($body, ['accessor']);
        $params = ['body' => json_encode($body)];
        return $this->client->post('/v1/auth/token/revoke-accessor', $params);
    }
